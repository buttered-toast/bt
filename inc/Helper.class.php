...
Class Helper {
  ...
  static public function style_enqueue_builder ($enqueue_args, $files_version) {
    foreach ($enqueue_args as $enqueue_arg) {
      if ($enqueue_arg['condition']) {
        wp_enqueue_style($enqueue_arg['handle'], TEMPLATE_DIRECTORY_URI . $enqueue_arg['url'] . $files_version);

        if (isset($enqueue_arg['url_ltr']) && !IS_RTL) {
          wp_enqueue_style($enqueue_arg['handle'] . '-ltr', TEMPLATE_DIRECTORY_URI . $enqueue_arg['url_ltr'] . $files_version);
        }
  ...
  static public function script_enqueue_builder ($enqueue_args, $files_version) {
    foreach ($enqueue_args as $enqueue_arg) {
      if ($enqueue_arg['condition']) {
        if (isset($enqueue_arg['dependencies']) && !empty($enqueue_arg['dependencies']) && is_array($enqueue_arg['dependencies'])) {
          $dependencies = $enqueue_arg['dependencies'];
        } else {
          $dependencies = ['jquery'];
        }

        wp_enqueue_script($enqueue_arg['handle'], TEMPLATE_DIRECTORY_URI . $enqueue_arg['url'] . $files_version, $dependencies, '', true);

        if (isset($enqueue_arg['url_ltr']) && !IS_RTL) {
          wp_enqueue_script($enqueue_arg['handle'] . '-ltr', TEMPLATE_DIRECTORY_URI . $enqueue_arg['url_ltr'] . $files_version, $dependencies, '', true);
        }
